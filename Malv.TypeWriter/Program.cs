...
namespace Malv.TypeWriter
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            IProjectAnalyzer analyzer = manager.GetProject(@"../../../../Malv/Malv.csproj");
            AdhocWorkspace workspace = analyzer.GetWorkspace(false);
            var project = workspace.CurrentSolution.Projects.First(x => x.Name == "Malv");
            ...
            // 5) Load template
            string template = File.ReadAllText(@"../../../NativeCodeModel.nt");
        ...
        private static Project AddXmlDocumentation(Project project, Type type)
        {
            ...
            project = project.AddMetadataReference(
                MetadataReference.CreateFromFile(assemblyPath, documentation: assemblyDocProvider));
            return project;
        }
    }
}